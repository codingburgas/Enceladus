...
void Test::Draw()
{
	...
	case QUESTION1:
		...
		DrawText(questions[0].c_str(), 500, 0, 75, BLACK);
		for (int i = 0; i < 4; i++)
		{
	...
	case QUESTION2:
		DrawRectangleRec(Background, GREEN);
		DrawText(questions[1].c_str(), 500, 0, 75, BLACK);
		for (int i = 0; i < 4; i++)
		{
			DrawRectangleRec(answersButtons[i], BLACK);
	...
	case QUESTION3:
		DrawRectangleRec(Background, GREEN);
		DrawText(questions[2].c_str(), 500, 0, 75, BLACK);
		for (int i = 0; i < 4; i++)
		{
			DrawRectangleRec(answersButtons[i], BLACK);
		...
		break;
	case ENDRESULT:
		DrawRectangleRec(EndBackground, GREEN);
